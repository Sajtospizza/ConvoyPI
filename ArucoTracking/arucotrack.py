...
for i in range(0, 10):
    ...
    if len(corners) != 0:
        # Get positions
        tempcars = get_positions(corners, ids, border_coords)
    for (key, value) in tempcars.items():
        if not cars.__contains__(key):
            cars[key] = value
    time.sleep(0.1)
print("Cars detected:")
print(cars)

# Send cars for identification
client_socket.sendall(json.dumps(cars).encode("utf-8"))
